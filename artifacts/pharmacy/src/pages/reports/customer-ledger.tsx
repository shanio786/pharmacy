...
import {
  useGetCustomerLedgerReport,
  getGetCustomerLedgerReportQueryKey,
  useListCustomers,
} from "@workspace/api-client-react";
import type { CustomerLedgerReport } from "@workspace/api-client-react";
...
export default function CustomerLedgerPage() {
  ...
  const params = customerId ? { customerId: Number(customerId), dateFrom, dateTo } : ({ customerId: 0, dateFrom, dateTo } as { customerId: number; dateFrom: string; dateTo: string });

  const { data, isLoading, refetch } = useGetCustomerLedgerReport(params, {
    query: { queryKey: getGetCustomerLedgerReportQueryKey(params), enabled: searched && !!customerId },
  });
  const r = data as CustomerLedgerReport | undefined;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-2"><BookOpen className="h-8 w-8 text-blue-600" />Customer Ledger</h1>
          <p className="text-sm text-muted-foreground">Statement of account for a customer.</p>
        </div>
        ...
        <CardContent className="pt-6 grid grid-cols-1 sm:grid-cols-4 gap-4">
          <div className="sm:col-span-2">
            ...
            <Select value={customerId} onValueChange={setCustomerId}>
              ...
              <SelectContent>
                {(customers as Array<{ id: number; name: string; phone?: string | null }>).map((c) => (
          ...
          <div className="sm:col-span-4">
            <Button onClick={() => { setSearched(true); refetch(); }} disabled={!customerId} className="w-full sm:w-auto">
        ...
        <>
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-lg font-semibold">{r.customer.name}</div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <Card><CardContent className="pt-6"><div className="text-xs text-muted-foreground">Total Sales</div><div className="text-xl font-bold">{fmt(r.totalSales)}</div></CardContent></Card>
