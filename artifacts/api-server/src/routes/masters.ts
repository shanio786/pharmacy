...
router.post("/racks", requireAuth, async (req, res) => {
  const { name, description } = req.body as {
    name: string;
    description?: string;
  };
...
router.patch("/racks/:id", requireAuth, async (req, res) => {
  const id = Number(req.params["id"]);
  const { name, description } = req.body as {
    name?: string;
    description?: string;
...
router.delete("/racks/:id", requireAuth, async (req, res) => {
  const id = Number(req.params["id"]);
...
router.post("/generic-names", requireAuth, async (req, res) => {
  const { name } = req.body as { name: string };
  if (!name) {
    res.status(400).json({ error: "Name is required" });
    return;
...
router.patch("/generic-names/:id", requireAuth, async (req, res) => {
  const id = Number(req.params["id"]);
  const { name } = req.body as { name?: string };
  const [row] = await db
...
router.delete("/generic-names/:id", requireAuth, async (req, res) => {
  const id = Number(req.params["id"]);
